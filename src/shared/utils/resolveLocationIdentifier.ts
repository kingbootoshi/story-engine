...
/**
 * @file resolveLocationIdentifier.ts
 * Utility for resolving location identifiers passed from AI suggestions.
 *
 * The AI may reference locations by their *name* instead of the canonical UUID
 ...
 *      location collection and return the matching `id`.
 ...
 * AI prompts should include exact location names when referring to them.  If we
...
/**
 * Resolve a location identifier (UUID or name) to its UUID.
 *
 * @param identifier – Either the location UUID or its display name.
 * @param locations – Collection of locations used for name → id mapping.
 * @returns UUID string when resolved or `null` when the identifier is unknown.
 */
export function resolveLocationIdentifier(
  identifier: string,
  locations: Location[]
): string | null {
  ...
  const match = locations.find(
    (l) => l.name.toLowerCase() === identifier.trim().toLowerCase()
  );
